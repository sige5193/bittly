<template>
  <a-modal v-model="visible" 
    :title="$t('test.functionalNode.Variable.settingTitle')" 
    :okText="$t('button.ok')"
  ...
  >
    <a-form :label-col="{ span: 5 }" :wrapper-col="{ span: 17 }">
      <a-form-item :label="$t('test.functionalNode.Variable.varName')">
        <a-input v-model="options.name" />
      </a-form-item>
      <a-form-item :label="$t('test.functionalNode.Variable.inputDefaultValue')">
        <a-input v-model="options.defaultValue" />
      </a-form-item>
      <a-form-item :label="$t('test.functionalNode.Variable.expression')">
        <a-input v-model="options.expression" />
      </a-form-item>
    </a-form>
  </a-modal>
</template>
