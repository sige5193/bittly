<template>
  <div>
    ...
    <a-select class="w-100"
      ...
      :placeholder="$t('directive.communicator.serialport.deviceSelectPlaceholder')"
    ...
    <a-modal v-if="devNameEditEnable" :visible="devNameEditEnable" 
      :title="$t('directive.communicator.serialport.webSerialDeviceNameEdit')"
      :okText="$t('button.ok')"
      :cancelText="$t('button.cancel')"
      @ok="actionDevNameEditOk"
    >
      <a-input v-model="devNameEditValue"
        :placeholder="$t('directive.communicator.serialport.webSerialDeviceNamePlaceholder')" 
      />
    </a-modal>
  </div>
</template>
...
export default {
    name : 'DirectiveComSerialportWebSerialDeviceSelector',
    components: {
        VNodes: {
            functional: true,
    ...
    methods : {
        ...
        editDeviceName() {
            ...
            return new Promise(( resolve, reject ) => {
                ...
                $this.devNameEditValue = `SP-${devId}`;
        ...
        actionDevNameEditOk() {
            if ( 0 === this.devNameEditValue.trim().length ) {
                let messageKey = 'directive.communicator.serialport.webSerialDeviceNameCannotBeEmpty';
